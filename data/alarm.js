...
function GotOScope(data) {
	...
	var fcanvasRect = fcanvas.getBoundingClientRect();
	
	if( otx.canvas.width != ocanvasRect.width )   otx.canvas.width = ocanvasRect.width;
	if( otx.canvas.height != ocanvasRect.height ) otx.canvas.height = ocanvasRect.height;

	if( ftx.canvas.width != fcanvasRect.width )   ftx.canvas.width = fcanvasRect.width;
	if( ftx.canvas.height != fcanvasRect.height ) ftx.canvas.height = fcanvasRect.height;
	...
	var fftsamps = secs[3];
	var iratio = ocanvasRect.height / ( secs[4]*4096 ) * 10;
	var data = secs[5];
	var fftdata = secs[6];
	...
	for( var round=0 ; round < channels ; round++ ) {
		...
		{
			var x2 = ((i-(samps * round)) ) * ocanvasRect.width / ( samps - 1 );
			var samp = parseInt(data.substr(i * 3, 3), 16);
			var y2 = ( 1.-mult*samp / 4096 ) * ocanvasRect.height ;
			
			if( i == 0 )
			{
				var x1 = i * ocanvasRect.width / samps;
				var y1 = ( 1.-mult*lastsamp / 4096 ) * ocanvasRect.height ;
				otx.moveTo( x1, y1 + ( ( mult * ocanvasRect.height ) / 2 ) - ( ocanvasRect.height / 2 ) );
			}

			otx.lineTo( x2, y2  + ( ( mult * ocanvasRect.height ) / 2 ) - ( ocanvasRect.height / 2 ) );
	...
	for(let f = 0; f <= 500; f += 50) {
		let x = (f * fcanvasRect.width) / 500;
		ftx.moveTo(x, fcanvasRect.height - 20);
		ftx.lineTo(x, fcanvasRect.height - 15);
		ftx.font = "12px Arial";
		ftx.fillText(f + "Hz", x - 15, fcanvasRect.height - 5);
	}
	ftx.stroke();
	
	const barWidth = fcanvasRect.width / fftsamps;
	...
	for( var round=0 ; round < channels ; round++ ) {
		...
		for (var i = fftsamps * round ; i < fftsamps * round + fftsamps ; i++) {
			var x = ((i-(fftsamps * round)) ) * fcanvasRect.width / fftsamps;
			var magnitude = parseInt(fftdata.substr(i * 3, 3), 16) * mult;
			var barHeight = (magnitude / 1024) * (fcanvasRect.height - 4);
			
			ftx.fillRect(x, fcanvasRect.height - 20 - barHeight, barWidth - 1, barHeight);
		}
	}
	...
	ftx.beginPath();
	ftx.strokeStyle = "#000000";
	ftx.strokeRect( 0,  0, fcanvasRect.width, fcanvasRect.height );
	ftx.stroke();
	
	otx.beginPath();
	otx.strokeStyle = "#c0c0c0";

    for ( i = 1; ( iratio * mult * i ) < ( ocanvasRect.height / 2 ); i++ ) {
		otx.moveTo(0, ocanvasRect.height / 2 + (iratio * mult * i));
		otx.lineTo(otx.canvas.width, ocanvasRect.height / 2 + ( iratio * mult * i ) );
		otx.moveTo(0, ocanvasRect.height / 2 - (iratio * mult * i));
		otx.lineTo(otx.canvas.width, ocanvasRect.height / 2 - ( iratio * mult * i ) );
	}
	...
	otx.strokeStyle = "#c0c0c0";
	otx.moveTo(0, ocanvasRect.height / 2);
	otx.lineTo(ocanvasRect.width, ocanvasRect.height / 2);

	otx.moveTo(1, ocanvasRect.height / 2 - 20 );
	otx.lineTo(1, ocanvasRect.height / 2 + 20);

	otx.moveTo(otx.canvas.width / 8, ocanvasRect.height / 2 - 10);
	otx.lineTo(otx.canvas.width / 8 , ocanvasRect.height / 2 + 10);

	otx.moveTo(otx.canvas.width / 8 * 2, ocanvasRect.height / 2 - 10 );
	otx.lineTo(otx.canvas.width / 8 * 2, ocanvasRect.height / 2 + 10);

	otx.moveTo(otx.canvas.width / 8 * 3, ocanvasRect.height / 2 - 10 );
	otx.lineTo(otx.canvas.width / 8 * 3, ocanvasRect.height / 2 + 10);

	otx.moveTo(otx.canvas.width / 2, ocanvasRect.height / 2 - 20);
	otx.lineTo(otx.canvas.width / 2 , ocanvasRect.height / 2 + 20);

	otx.moveTo(otx.canvas.width / 8 * 5, ocanvasRect.height / 2 - 10 );
	otx.lineTo(otx.canvas.width / 8 * 5, ocanvasRect.height / 2 + 10);

	otx.moveTo(otx.canvas.width / 8 * 6, ocanvasRect.height / 2 - 10);
	otx.lineTo(otx.canvas.width / 8 * 6, ocanvasRect.height / 2 + 10);

	otx.moveTo(otx.canvas.width / 8 * 7, ocanvasRect.height / 2 - 10);
	otx.lineTo(otx.canvas.width / 8 * 7, ocanvasRect.height / 2 + 10);

	otx.moveTo(otx.canvas.width - 1, ocanvasRect.height / 2 - 20 );
	otx.lineTo(otx.canvas.width - 1, ocanvasRect.height / 2 + 20);

	otx.strokeStyle = "#000000";
	otx.strokeRect( 0,  0, ocanvasRect.width, ocanvasRect.height );
